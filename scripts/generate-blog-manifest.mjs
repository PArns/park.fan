...
/**
 * A link destination is where a post breaks silently and loudly at once.
 * CommonMark ends `](…)` at the paren that balances the opening one, so a URL
 * carrying a stray `(` — Movie Park's own CMS serves its bonus-club PDFs from a
 * folder called `vorteile-saisonpassinhaber-(bonusclub(` — ends the link early:
 * the label renders as literal `[text]`, and the tail of the URL is autolinked
 * as its own link text, which is a few hundred unbreakable characters sitting in
 * a paragraph. Angle brackets take the whole thing verbatim: `](<https://…>)`.
 */
function warnAboutLinkDestinations(posts) {
  const warnings = [];
  ...
  for (const post of posts) {
    ...
    for (let i = body.indexOf(']('); i !== -1; i = body.indexOf('](', i + 2)) {
      // `](<…>)` is the fix, not a finding — inside angle brackets a paren is
      // just a character.
      ...
      if (depth === 0) continue;
      // `](url "title")` legitimately puts a space in there. No post uses the
      // form today; it must not become a false positive if one ever does.
      if (/^\s*["']/.test(body.slice(j))) continue;

      const dest = body.slice(i + 2, j);
      warnings.push(
        `${file}: link destination "${dest.length > 80 ? `${dest.slice(0, 80)}…` : dest}" ` +
          'has an unbalanced "(" — markdown ends the link there. Wrap it: ](<…>)'
      );
    }
  }
...
const linkDestinationWarnings = warnAboutLinkDestinations(posts);
...
console.log(
  `📝 Generated blog manifest: ${posts.length} posts (${kb(OUTPUT_META)} KB meta, ` +
    ...
    (catalogLinkWarnings > 0
      ...
      : '') +
    (linkDestinationWarnings > 0
      ? `\n   ⚠️  ${linkDestinationWarnings} broken link destination(s) above`
      : '')
);
