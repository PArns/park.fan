 ...
 *
 * Set by the field-capture route, which writes a sidecar from what a phone can
 * know on the spot — park, ride, area, credit, capture date — and leaves the
 * things that need the picture and a keyboard: alt text, caption, the tags that
 * describe what is in frame. Without a flag those photos are indistinguishable
 * from finished ones, because "no alt text yet" and "this image needs none" look
 * the same in the database.
 *
 * Deliberately a field of its own rather than a `needs-review` tag: the tag
 * vocabulary describes what a photo SHOWS, and a workflow state filed among the
 * subjects is a state that survives into the published caption search.
 *
 * `false` and absent mean the same thing, and only `true` is ever written, so the
 * ~900 sidecars that predate this are byte-identical after a round trip.
 */
function asReview(value, issues) {
  if (value === null || value === undefined || value === false) return false;
  if (value === true) return true;
  issues.push(`review: expected true or false, got "${value}"`);
  return false;
}
...
export function normalizeSidecar(raw) {
  ...
  return {
    sidecar: {
      ...
      review: asReview(source.review, issues),
    },
    text: {
...
/**
 * `printWidth` from `.prettierrc`, which this file cannot read: it is bundled
 * into a serverless function, where the config is not on disk. `scripts/test-media.mjs`
 * asserts the two agree, so changing `.prettierrc` turns that test red instead of
 * turning the next media session's commit red.
 */
...
/**
 * Code point ranges Prettier counts as two columns: East Asian Wide and Fullwidth,
 * plus the emoji blocks. Everything else it counts as one, and combining marks as
 * none.
 *
 * This matters because the width that decides a line break is the width on screen,
 * not the number of UTF-16 units. `'一'.length` is 1 and `'😀'.length` is 2, while
 * both occupy two columns — so a tag in a CJK script measured with `.length` reads
 * up to 38 columns narrower than it prints, and an array Prettier breaks would be
 * written on one line. Tags are the only array here that is not slug-validated, so
 * they are the only place a character like this can reach the file.
 */
const WIDE_RANGES = [
  [0x1100, 0x115f],
  [0x2e80, 0x303e],
...
/** Marks that print on top of the character before them, so they add no column. */
...
/** Columns a string occupies on screen, counted the way Prettier counts them. */
...
/**
 * Print a value the way Prettier prints JSON, for the three shapes a sidecar
 * holds: primitives, flat objects of primitives, and flat arrays of primitives.
 *
 * `JSON.stringify(value, null, 2)` is NOT that shape, and the difference is one
 * rule. Objects agree, because Prettier keeps an object expanded when its source
 * has a line break after the brace, and `JSON.stringify` always writes one. Arrays
 * do not: Prettier pulls an array onto a single line whenever that line fits
 * within `printWidth`, and `JSON.stringify` always breaks it across lines.
 *
 * `prettier --check .` covers `public/media/`, so that one rule made every sidecar
 * written here a file the check rejects — 157 of 157 measured on 2026-09-21. The
 * media session of `f5d6f5e1` landed 13 of them on `main` and turned
 * `lint-and-format` red for every pull request open at the time, none of whose
 * diffs had anything to do with media.
 *
 * `prefix` is what already stands on the line in front of this value, and `tail`
 * what follows it there (the comma after a key that is not the last one). Prettier
 * counts both when it decides whether an array fits, so this does too.
 */
function printJson(value, depth, prefix, tail) {
  if (Array.isArray(value)) {
    const parts = value.map((entry) => JSON.stringify(entry));
    const inline = `[${parts.join(', ')}]`;
    if (visualWidth(prefix + inline + tail) <= PRINT_WIDTH) return inline;
  ...
  if (value !== null && typeof value === 'object') {
    ...
    const lines = keys.map((key, index) => {
      const keyPrefix = `${pad}${JSON.stringify(key)}: `;
      const keyTail = index < keys.length - 1 ? ',' : '';
      return keyPrefix + printJson(value[key], depth + 1, keyPrefix, keyTail) + keyTail;
    });
    return `{\n${lines.join('\n')}\n${INDENT.repeat(depth)}}`;
  }
 ...
 * empty values omitted, printed the way Prettier would print it. What the admin
 * browser writes, what a human would hand-author and what `prettier --write`
 * would leave behind are then byte-identical, so saving from the UI produces a
 * clean diff and a green `format:check`.
 */
export function serializeSidecar(sidecar, text) {
  ...
  // Last, and only when set: it says something about the RECORD, not the picture,
  // and writing `false` would rewrite every sidecar in the repository for nothing.
  if (sidecar.review) out.review = true;
