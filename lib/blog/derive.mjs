...
export function extractInlineRefs(markdown) {
  ...
  while ((match = widgetRe.exec(markdown)) !== null) {
    ...
    // A fence may name its park the same way a `ref:` link does — `/parks/<continent>/<country>/
    // <city>/<park>` — for the one slug that means two parks (`disneyland-park`). Normalise here
    // too, or the backlink index records the whole path as a park slug and the manifest
    // generator rejects it.
    // First geoPath wins, like the ride branch above: a post that names both Disneyland Parks
    // would otherwise have its Paris entry overwritten by whichever reference came last, and the
    // backlink would land on the wrong park page.
...
/**
 * The glossary terms one post's body points at, in first-appearance order.
 *
 * Only `glossary-widget` fences count, and that is the whole design decision here. A post also
 * links into the glossary through `GlossaryInject`, which highlights every known term and alias on
 * its first occurrence in the prose — but that is a reading aid, not a topical claim. Counting it
 * would put nearly every post on `wartezeit`, `warteschlange` and a dozen other everyday words,
 * and the reverse index exists precisely to answer "which articles are ABOUT this term".
 * Embedding the widget is the author saying so deliberately.
 *
 * The fence's `slug=` is a term ID, not a localized slug (`check-blog-slugs.mjs` validates it
 * against the ids in `content/glossary/en.ts`), so one post's refs answer for all six locales.
 *
 * @param {string} markdown
 * @returns {string[]}
 */
