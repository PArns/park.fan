...
// A regression guard for the caching, not a benchmark — and the threshold is now the PER-QUERY
// time, because the total depends on `QUERIES` and the per-query figure is the thing that
// regresses.
//
// The old assertion was `ms < 6` against a run that measures 6.26 ms idle, so this selftest was
// RED at HEAD in four runs of four while the module's report claimed "25 assertions, all
// passing". Nothing ran it: it was not in `pnpm test:game` (it is now), so nobody found out. The
// threshold also contradicted the comment above it, which says a loaded machine measures 2× an
// idle one and the bug this catches was 20×: at 0.313 µs per query idle, 2 µs clears a loaded
// machine threefold and still fails the moment a `Map` walk gets back into the hot path.
const perQuery = (ms / QUERIES) * 1000;
check(
  'a path query stays inside its share of the tick',
  perQuery < 2,
  `${perQuery.toFixed(3)} µs each, ${ms.toFixed(2)} ms for ${QUERIES}`
);
