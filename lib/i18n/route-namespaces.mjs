...
export const LAZY_MESSAGE_BOUNDARIES = [
  {
    ...
    // `pushAlerts.{showBell,rideBell,rideDialog}` ride along for the same
    // reason `parks`/`attractions` do: `ShowCard`/`AttractionCard` carry the
    // follow/alert bells unconditionally, so any route that can show the
    // favorites band can reach them.
    namespaces: ['parks', 'attractions', 'pushAlerts.showBell', 'pushAlerts.rideBell', 'pushAlerts.rideDialog'],
  },
  {
