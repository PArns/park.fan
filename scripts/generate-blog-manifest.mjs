 ...
 *   lib/blog/manifest-galleries.ts BLOG_GALLERY_FOLDERS (+ _L10N) — every
 *                                  /public/blog/images/<folder>/ image listing
 *                                  with its captions.json overrides, and the
 *                                  per-locale captions.<locale>.json ones
 *                                  (consumers fall back to the base list).
 */
...
import { calcReadingTimeMinutes, extractParkRefs } from '../lib/blog/derive.mjs';
...
const OUTPUT_GALLERIES = resolve(rootDir, 'lib/blog/manifest-galleries.ts');
...
const galleriesModule = `import type { BlogImage } from './types';
...
writeFileSync(OUTPUT_GALLERIES, banner + galleriesModule);
...
console.log(
  `📝 Generated blog manifest: ${posts.length} posts (${kb(OUTPUT_META)} KB meta, ` +
    `${kb(OUTPUT_BODIES)} KB bodies), ${Object.keys(galleries).length / 2} gallery folders ` +
    `(${kb(OUTPUT_GALLERIES)} KB) → ${relative(rootDir, OUTPUT_META)}`
);
