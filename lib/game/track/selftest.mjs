...
// footprint has to be the real one, or the build bar offers a ghost the size of a lie.
{
  ...
  for (const entry of declared) {
    ...
    /**
     * The footprint's SIZE was right from the first day and its POSITION was never checked, which
     * is a different lie and a worse one. This module builds from the origin outwards, so the box
     * is metres away from the point the player clicked; `tools` centred the ghost on the pointer
     * and `evaluatePlacement` judged that. Measured on the first coaster ever placed by a click:
     * 600 of 2001 track samples underground, 1.79 m into the hill, on a rectangle that had been
     * approved. `footprintOffset` closes it and this is what keeps it closed -- edit a piece and
     * the offset moves, so it is measured here rather than trusted.
     */
    const offset = entry.footprintOffset ?? [0, 0];
    near(offset[0], (Math.max(...xs) + Math.min(...xs)) / 2, 1, `"${entry.id}" footprint offset x`);
    near(offset[1], (Math.max(...zs) + Math.min(...zs)) / 2, 1, `"${entry.id}" footprint offset z`);
    ok(
      Math.hypot(offset[0], offset[1]) > 1,
      `"${entry.id}" declares where its box sits relative to the anchor`,
      `[${offset[0]}, ${offset[1]}]`
    );
    /**
     * And how far BELOW the anchor the layout reaches, which is the other half of the same lie.
     * Fixing the offset put the box exactly where the ghost drew it and left 621 of 2001 track
     * samples inside the hill, because every one of these layouts dips under its own station --
     * `nordwind` by 6 m -- and `tools` was dropping the anchor onto the terrain. It lifts the
     * anchor by this figure now, so it has to be the measured one.
     */
    near(entry.footprintDip ?? 0, Math.max(0, -Math.min(...ys)), 0.2, `"${entry.id}" footprint dip`);
