...
const translations: GlossaryTermTranslation[] = [
  {
    ...
    definition:
      'Wait time (also called queue time) is the estimated duration a guest must stand in line before boarding a ride or attraction. Parks post wait times at ride entrances and on their official apps, calculating the figure from queue length sensors, historical throughput data, and current loading efficiency. On busy days a single popular attraction can show waits of 90 minutes or more.\n\npark.fan re-reads the wait times every five minutes, for every attraction in a park. Set side by side at the same moment, they show which rides are in a temporary lull and which have built up a long queue.',
  ...
  },
  {
    ...
    definition:
      'Single rider queues allow guests who are comfortable riding alone — or separated from their group — to fill odd seats left over in ride vehicles. Because single riders slot into gaps rather than waiting for an entire row to fill, the line moves faster than the standby queue, often cutting wait times by 50–70%. Rides with large multi-row vehicles, such as roller coasters and simulator attractions, benefit the most from this system.\n\nNot every park or attraction offers single rider access. Where available it is one of the most cost-free strategies for reducing your daily queue time. The trade-off is that you may not sit alongside your companions, so it works best for larger thrill rides where the experience is largely individual. Whether a ride runs the lane on a given day is announced at its entrance and in the park app.',
