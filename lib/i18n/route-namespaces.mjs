...
/** Route files that make a segment renderable, in addition to the layout chain. */
const ROUTE_FILES = ['page.tsx', 'template.tsx', 'error.tsx', 'loading.tsx', 'not-found.tsx'];
...
export function analyzeRouteNamespaces({ root = process.cwd() } = {}) {
  ...
  /**
   * Modules reachable from `entries`. A lazy boundary is itself reachable (it
   * still renders), but the walk does not continue THROUGH it — its subtree
  ...
  function reachableFrom(entries) {
    ...
    while (stack.length > 0) {
      ...
      seen.add(file);
      if (lazyFiles.has(file)) continue;
      for (const imported of read(file).imports) {
  ...
  function entryFilesFor(routeDir) {
    const entries = [];
    for (const name of ROUTE_FILES) {
      const file = path.join(root, routeDir, name);
      if (fs.existsSync(file)) entries.push(file);
    }
    let dir = routeDir;
    while (dir.startsWith('app')) {
      const layout = path.join(root, dir, 'layout.tsx');
      if (fs.existsSync(layout)) entries.push(layout);
      if (dir === 'app') break;
  ...
  // 2. The layout chain alone — what the chrome (header, footer, search, banner)
  //    needs. Every route pays this, so it stays in the root provider.
  const localeLayoutDir = path.join('app', '[locale]');
  const layoutEntries = entryFilesFor(localeLayoutDir).filter((f) => f.endsWith('layout.tsx'));
  const layout = collapseNamespaces(namespacesIn(reachableFrom(layoutEntries)).keys());
  ...
  for (const routeDir of routeDirs) {
    const found = namespacesIn(reachableFrom(entryFilesFor(routeDir)));
    ...
    const delta = required.filter(
      (ns) => !layout.some((base) => ns === base || ns.startsWith(base + '.'))
    );
  ...
  return { layout, routes, lazy: [...LAZY_CHUNK_NAMESPACES], consumers };
}
