...
export function extractInlineRefs(markdown) {
  ...
  while ((match = widgetRe.exec(markdown)) !== null) {
    ...
    if ((name === 'park-widget' || name === 'map-widget') && slug) {
      ...
      parks.add(ref.key);
      if (ref.geoPath) parkGeoPaths.set(ref.key, ref.geoPath);
    } else if (name === 'attraction-widget' && slug && parkAttr) {
      const ref = parseRefKey(parkAttr);
      parks.add(ref.key);
      attractions.add(`${ref.key}/${slug}`);
      if (ref.geoPath) {
        parkGeoPaths.set(ref.key, ref.geoPath);
        attractionGeoPaths.set(`${ref.key}/${slug}`, ref.geoPath);
      }
    }
  }
