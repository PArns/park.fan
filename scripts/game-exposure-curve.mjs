...
await p.goto(
  `http://localhost:3000/game?harness=1&speed=0&engine=webgl2&showcase=environment${weather ? '&weather=' + weather : ''}`,
...
for (const r of rows) {
  ...
  console.log(
    `${r.at}   ${String(r.exposure).padStart(6)}   ${pin}    ${String(r.contrast).padStart(5)}  ${String(r.sunIntensity).padStart(6)}  ${String(r.night).padStart(4)} ${String(r.cloud).padStart(4)}  ${r.fog}`
  );
}
