...
section('1 · block plan');
{
  ...
  ok(
    none.blocks.length === 0 && none.station === -1,
    'a layout with nothing that holds has no plan'
  );
}
...
section('2 · integrator');
{
  ...
  for (let i = 0; i < 400 && !parked; i++) {
    ...
    parked = stepTrain(
      held,
      { ...holdCtx, sampler: { ...sampler, gradeAt: () => 0 } },
      hold,
...
section('3 · integrator vs simulateTrack');
{
  ...
  for (const preset of TRACK_LAYOUTS) {
    ...
    let seconds = 0;
    const lap =
      built.spline.length() - (plan.blocks[plan.station].stop - plan.blocks[plan.station].from);
    let travelled = 0;
...

{
  ...
  // Derived from the layout table, not typed: this read `=== 3` and went red the day a fourth
  // bundled layout landed, which is a test failing on arithmetic rather than on behaviour.
  ok(
    api.ids().length === TRACK_LAYOUTS.length,
    `one fleet per bundled layout (${TRACK_LAYOUTS.length})`,
    JSON.stringify(api.ids())
  );
  ...
  ok(statuses.find((s) => s.blocks === 4)?.trains === 3, 'the four-block layout runs three trains');
  rt.dispose();
}

{
  ...
  ok(
    errors.length === 0,
    'no sim errors over 800 ride seconds',
...

{
  ...
  ok(
    minGap > 25,
    'and the closest two trains ever come is more than a train length',
    `${minGap.toFixed(1)} m`
  );
...
section('6 · state');
{
  ...
  ok(
    anyFleet &&
      typeof anyFleet.sinceDispatch === 'number' &&
      typeof anyFleet.dispatches === 'number',
