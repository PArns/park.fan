...
import { DOCKED_MOTION_STRIDE, MOTION_STRIDE, RideState } from '@/lib/game/rides/types.ts';
 ...
 * event.
 */
function frameFor(rides, guests, stats = {}, docked = []) {
  ...
  const dstate = new Uint8Array(docked.length);
  const dmotion = new Float32Array(docked.length * DOCKED_MOTION_STRIDE);
  docked.forEach((m, i) => {
    dstate[i] = m.state;
    dmotion[i * DOCKED_MOTION_STRIDE + 0] = m.riders;
    dmotion[i * DOCKED_MOTION_STRIDE + 1] = m.queue;
  });
  ...
  return {
    ...
    buffers: {
      ...
      'rides.dockedState': dstate.buffer,
      'rides.dockedMotion': dmotion.buffer,
...
ok('a coaster is in the list AND in the count, and the two agree', () => {
  // The bug this pins: `ride:roster` is the machines `rides` DRAWS, so a park with a coaster in it
  // listed four rides and reported `ridersToday` from six. Measured on the demo park at 16:33 on
  // day 1 it read "RUNNING 3 / 4" over 1,842 rides taken. The docked machines arrive on their own
  // roster key and their own frame buffers, so the list and the totals describe one set again.
  const c = collector();
  c.onRoster(
    [
      { id: 'r1', key: 'core-classic:carousel' },
    ...
    [
      {
        id: 'coaster-1',
        key: 'core-classic:family-invert',
        name: { en: 'Family invert', de: 'Familien-Invert' },
        dispatchedBy: 'trains',
        capacity: 24,
      },
      {
        id: 'flume-1',
        key: 'neon-lagoon:tube-slide',
        name: { en: 'Tube slide' },
        dispatchedBy: 'flumes',
        capacity: 4,
      },
  ...
  c.onFrame(
    frameFor(
      ...
      { 'rides.count': 4 },
      [
        { state: RideState.RUNNING, riders: 20, queue: 31 },
        { state: RideState.CLOSED, riders: 0, queue: 0 },
      ]
    )
  );
  const s = c.snapshot();
  assert.equal(s.rides.length, 4, 'two drawn machines and two docked ones');
  assert.equal(s.totals.rides, 4, 'and the headline count is the same four');
  // The drawn rows keep their index against `rides.motion`; the docked ones are appended.
  assert.equal(s.rides[0].dispatchedBy, 'rides');
  assert.equal(s.rides[1].dispatchedBy, 'rides');
  assert.equal(s.rides[2].id, 'coaster-1');
  assert.equal(s.rides[2].name, 'Family invert', 'the name travels on the roster, not the id');
  assert.equal(s.rides[2].dispatchedBy, 'trains');
  assert.equal(s.rides[2].state, 'running');
  assert.equal(s.rides[2].riders, 20);
  assert.equal(s.rides[2].queue, 31);
  assert.equal(s.rides[2].capacity, 24);
  assert.equal(s.rides[3].id, 'flume-1');
  assert.equal(s.rides[3].state, 'closed');
  // Two of four open — the broken flat ride and the closed slide are not — and the queue is the
  // sum of all four rather than of the two this module draws.
  assert.equal(s.totals.ridesOpen, 2, 'running + running; broken and closed are not open');
  assert.equal(s.totals.ridesDown, 1, 'the broken flat ride');
  assert.equal(s.totals.queued, 42 + 7 + 31);
  assert.equal(s.totals.riding, 18 + 20);
  // A figure this side cannot honestly answer for a docked machine stays 0 rather than invented.
  assert.equal(s.rides[2].excitement, 0);
  assert.equal(s.rides[2].price, 0);
});

ok('a roster with no docked key still lists the drawn machines', () => {
  // `docked` defaults to empty on both sides, so a worker built before the key existed publishes
  // a roster this side accepts unchanged.
  const c = collector();
  c.onRoster([{ id: 'r1', key: 'core-classic:carousel' }]);
  c.onFrame(frameFor([{ state: RideState.RUNNING, riders: 3, queue: 4 }], []));
  const s = c.snapshot();
  assert.equal(s.rides.length, 1);
  assert.equal(s.rides[0].dispatchedBy, 'rides');
});
...
ok('the clock comes from the world, not from the last frame', () => {
  const world = createWorld({ seed: 1, packs: [] });
  const c = new TelemetryCollector({
    world,
    registry: new Registry(),
    locale: 'en',
    rideProfile: () => undefined,
  });
  // A frame whose clock says one thing while the world says another: `setTimeOfDay()` writes the
  // world and the harness does it constantly, so the world is the one a reader is looking at.
  world.clock.day = 4;
  world.clock.minute = 780;
  world.clock.speed = 0;
  c.onFrame(frameFor([], []));
  const s = c.snapshot();
  assert.equal(s.day, 4);
  assert.equal(s.minute, 780);
