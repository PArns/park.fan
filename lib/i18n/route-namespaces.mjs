...
export const LAZY_MESSAGE_BOUNDARIES = [
  {
    ...
    // `pushAlerts.{showBell,showDialog,rideBell,rideDialog}` ride along for the
    // same reason `parks`/`attractions` do: `ShowCard`/`AttractionCard` carry
    // the follow/alert bells unconditionally, so any route that can show the
    // favorites band can reach them. `showDialog` joined once a showtime badge
    // on `ShowCard` started opening `ShowFollowDialog`.
    //
    // `nearby` and `parkCard` are `ParkCard`'s own (`park-card.tsx`,
    // `park-time-info.tsx`, `park-card-schedule-footer.tsx`, `park-distance.tsx`)
    // and belong here for exactly as long as this band has rendered park cards.
    // They were missing until `/favorites` — every earlier host of the band also
    // renders a nearby-parks section, which ships both eagerly, so the omission
    // resolved through the page instead of through this list and nothing said so.
    // Costs 577 B brotli on the fetched chunk, and nothing at all on a route that
    // already carries the two (`useLazyMessages` resolves those without a request).
    namespaces: [
      'parks',
      'attractions',
      'nearby',
  ...
  },
  {
    ...
    // day-detail dialog, so the chunk is fetched when that opens — and the park
    // page plus all 27,984 calendar URLs keep the namespace out of their payload.
