...
| `test-rider-height.mjs`                | Unit tests for the park page's rider-height filter: that a ride with no height on file stays rideable, that both bounds include their own value, and that `maximumHeight` filters without stretching the slider's range. `pnpm test:rider-height`, see [attraction filter panel](../features/attraction-filter-panel.md)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `test-attraction-history-geometry.mjs` | Unit tests for the ride page's 30-day history calendar: that the 31-day window spans five week rows or six depending only on the weekday it starts on, that the below-`lg` list height never varies, and that a year boundary, a leap day and a DST Sunday do not shift the count. The placeholder and the grid reserve one box from these numbers, so a wrong row count is a shift on every ride page at once. `pnpm test:attraction-history-geometry`, see [design system](../design/design-system.md#the-rides-calendar-cell-is-parkcalendarday-plus-a-sparkline)                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| `generate-icons.mjs`                   | Cuts the whole icon set from the brand files. Two sources, split by the smallest size a surface may draw each file at — not by the file's own size: `logo-small-dark.svg` (the header's pin) for `app/favicon.ico` (16/32/48/96), `/icon.svg` and the 180 px Apple touch icon, which Google may scale to 16 px itself; `logo-big-dark.svg` — the lockup, wordmark clipped off by the measurement — for the manifest's 192/512 and its maskable 512. It reads the lockup rather than the already-cut `logo-dark.svg` so the icon set does not depend on a second generator's output. Ink box and the wordmark's position are measured by rendering, never typed. `pnpm generate:icons` writes, `pnpm check:icons` fails on drift; see [favicon](../seo/favicon.md)                                                                                                                                                                                                                                                         |
| `generate-brand-pin.mjs`               | Cuts the detailed pin out of `logo-big*.svg` into `logo.svg` / `logo-dark.svg`, which the footer draws. Those two were the last raster in the brand set — a 1563×1116 PNG in an SVG wrapper, 77 KB each — and nothing looked wrong at the 32/48 px asked of them, which is why they lasted. The wordmark goes by the exporter's own id (`<g id="park.fan">`, against `<g id="Icon">` for the pin), unreferenced gradients go with it, and the `viewBox` is the ink box, measured by rendering. 6.2 KB each. `pnpm generate:brand-pin` writes, `pnpm check:brand-pin` fails when a re-exported lockup leaves the pin behind                                                                                                                                                                                                                                                                                                                                                                                                |
| `check-prose.mjs`                      | The writing rules in [docs/blog.md](../blog.md), as far as a regex can decide them: posts, the six message catalogs and every media sidecar. Errors are rules with no legitimate exception (a `—` in a post body, an honesty claim or chat register in a string that can only be about us, a growing em-dash count in a catalog — the catalogs are ratcheted rather than judged, because 217 strings predate the rule); warnings are budgets and signals a person decides on, because `eerlijke prijzen` means _fair_ prices. It also prints sentence-length variance and comma density per locale. Found `Ehrliche Reiseberichte` on the blog index in all six languages on its first run. `pnpm check:prose`, `--strict` to fail on warnings                                                                                                                                                                                                                                                                            |
