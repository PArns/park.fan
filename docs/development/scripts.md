...
| `generate-icons.mjs`               | Cuts the whole icon set — `app/favicon.ico` (16/32/48/96), `/icon.svg`, the Apple touch icon, the manifest's 192/512 and its maskable 512 — from one source, `public/logo-small-dark.svg`. The ink bounding box is measured by rendering, never typed, so a re-export at different margins cannot silently shrink the favicon. `pnpm generate:icons` writes, `pnpm check:icons` fails on drift; see [favicon](../seo/favicon.md)                                                                                                                                                                                                                                                                                                                              |
