...
/**
 * Compact "live now" header for the attraction page's unified live card. Shows
 * the current standby wait as the centerpiece, with status, last-updated and the
 * KI-accuracy chip on the right. Today's wait progression is no longer a
 * sparkline here — the full "Wartezeiten heute" bar chart sits directly below
 * this header inside the same card, so the value and the chart read as one unit.
 *
 * Renders bare (no card chrome) — the parent card provides the border/background.
 *
...
export function AttractionLivePanel({
  ...
  return (
    <div className="flex flex-col gap-5 p-4 sm:flex-row sm:items-center sm:justify-between sm:gap-8 sm:p-6">
      {/* Left: live wait time */}
