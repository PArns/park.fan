...
export async function generateStaticParams() {
  try {
    // Fetch all available continents by trying to get continent details
    ...
    for (const continent of POSSIBLE_CONTINENTS) {
      try {
        ...
        // Continent doesn't exist, skip it
        continue;
      }
    }
    ...
    console.error('Error generating static params for continents:', error);
    return [];
  }
}
...
export default async function ContinentPage({ params }: ContinentPageProps) {
  ...
  try {
    ...
    const parksByCountry = data.data.reduce((acc, park) => {
      if (!acc[park.country]) {
        acc[park.country] = [];
      }
    ...
    return (
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-8">
        <div className="mb-6 lg:mb-8">
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold mb-3 lg:mb-4">{continentName} Theme Parks</h1>
          ...
          {Object.entries(parksByCountry).map(([country, parks]) => (
            <div key={country}>
              <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-2 mb-4">
                <h2 className="text-xl sm:text-2xl font-semibold">{getCountryFlag(country)} {country}</h2>
                <Badge variant="outline" className="text-xs w-fit">{parks.length} park{parks.length !== 1 ? 's' : ''}</Badge>
              </div>
                    ...
                    <Card className="h-full hover:shadow-lg transition-shadow">
                      <CardHeader className="pb-3">
                        <div className="flex items-center justify-between">
                          <CardTitle className="text-base sm:text-lg line-clamp-2 pr-2">{park.name}</CardTitle>
                          <Badge 
