...
| **Success means** | A day without meltdowns: short waits for the rides *her kids may actually ride*, a rain plan, food without drama |
...
| **Key questions** | "How full is it *right now* and until close?" · "Will the rain pass by 15:00?" · "Ping me when it's a green day" |
...
| **Success means** | Low walking distances, benches and quiet slots, rides the *whole* group can do together |
...
**Why these six:** P1/P5 stress *pre-visit content and safety metadata*, P2 stresses *data depth and multi-park tooling*, P3 stresses *real-time and proactive signals*, P4 stresses *guidance and i18n*, P6 stresses *SEO landing performance and conversion to retained user*. Together they cover the full lifecycle below.
...
*"Which park should we even go to?"* — mainly P1, P2, P4, P5.
...
*"Pick the date, sketch the day."* — P1, P2, P4, P5.
...
| **Ticket prices / links** | ❌ | Not surfaced at all — although backend already stores prices in `schedule_entries.purchases` and Lightning-Lane prices in `queue_data.price`. Frontend gap more than backend gap |
...
| **Save a planned trip (date + park)** | ❌ | Favorites exist (cookie, per device) but a *trip* (park + date) cannot be saved — which also blocks "your day got busier" notifications (G3) |
...
*"Is the plan still good?"* — P1, P3, P5.

| Need | Today | Notes |
| --- | --- | --- |
| 16-day weather forecast | ✅ | Forecast strip + hourly chart per park |
| Updated crowd prediction for the chosen day | ✅ | Calendar + Fancast; but the user must *actively re-check* |
...
*"Be smart in the first 90 minutes."* — all personas, most critical for P4 (first-timer).
...
| "How does this park work" primer | 🟡 | `/howto` + glossary explain concepts generically; there is no *per-park* first-visit guide ("at DLP, do X first, buy Y, the app you need is Z") |
...
*"What now?"* — all personas; P2/P3 heaviest users.
...
| Single-rider / virtual-queue visibility | 🟡 | Live queue types (SINGLE_RIDER, RETURN_TIME, BOARDING_GROUP incl. prices) are in the backend; frontend surfaces standby prominently, other queue types minimally |
| Restaurant depth (cuisine, reservation needed) | 🟡 | Backend has `cuisineType`, `cuisines[]`, `requiresReservation`, party-size wait data; frontend restaurants tab shows little of it |
| **Battery/data-friendly offline mode** | ❌ | No service worker; every glance is a full network round-trip (G4) |

### S-F · After the visit

*"Remember, share, return."* — P2 primarily; retention lever for all.

| Need | Today | Notes |
| --- | --- | --- |
| Contribute photos | ✅ | `/contribute` flow (Turnstile-gated) |
| Blog/editorial to stay engaged | ✅ | Full blog with rich embeds, RSS |
| **Ride log / visit diary** ("I rode 14 rides on 2026-07-12") | ❌ | Nothing. For P2 this is identity ("count"); requires accounts or at least local persistence. The wait-time history to *reconstruct* a day already exists server-side |
...
The pattern: **park.fan today is excellent at "when to go" and "what's happening now", for a data-literate visitor planning for themselves.** It underserves anyone planning *for someone else* (kids, grandparents), anyone needing *guidance* rather than data (first-timers), and every *proactive/retention* touchpoint (alerts, sync, offline).
...
| --- | --- | --- | --- | --- |
| **G1** | **Attraction metadata**: min height, intensity/thrill level, indoor/outdoor, accessibility (wheelchair/transfer), ride duration | P1, P5, P4 | ❌ **No** — `attractions` has only type string, land, seasonality. Needs new fields + a data source (manual curation, wiki, or park sites) | Unlocks: family filter ("rides for 110 cm"), rain plan (indoor filter), accessibility view, intensity badges. Single highest-leverage data investment; three scenarios blocked on it |
...
| **G5** | **Day-plan / touring guide**: generated ride order from rope-drop tiers + hourly forecast; per-park first-visit guide (editorial) | P4, P1, P2 | ✅ **Yes** — rope-drop tiers, headliners, PCN intraday forecasts, walking coordinates all exist | The biggest *product* differentiator on data park.fan already uniquely has. Editorial per-park guides can start without any code |
...
| **G8** | **Surface data the backend already has**: ticket/event prices (`schedule_entries.purchases`), Lightning-Lane prices (`queue_data.price`), single-rider & virtual-queue waits, restaurant cuisine/`requiresReservation`, `TICKETED_EVENT` as event badges | P1, P2, P4 | ✅ **Yes — already stored** | Pure frontend quick wins; the season/event badges are SEO roadmap Phase 4 |
...
- **G1 data sourcing:** manual curation per park (start with top 20), community contribution (the `/contribute` flow could be extended beyond photos), or licensing/scraping? Height limits and indoor/outdoor are stable, low-churn data — a one-time curation gets far.
