...
class TranslationCrawler {
          ...
          for (const country of continent.countries) {
            ...
            try {
              const citiesResponse = await fetch(
                `${API_BASE}/v1/discovery/continents/${continent.slug}/${country.slug}`
              );
              ...
              if (citiesResponse.ok) {
                    ...
                    for (const park of city.parks) {
                      ...
                      try {
                        const parkResponse = await fetch(
                          `${API_BASE}/v1/parks/${continent.slug}/${country.slug}/${city.slug}/${park.slug}`
                        );
                        if (parkResponse.ok) {
                          ...
                          const addSubRoutes = (items, type) => {
                            ...
                            if (sampledItems.length > 0) {
                              console.log(
                                `      + ${park.slug}: ${sampledItems.length} ${type} (sampled)`
                              );
                            }
                ...
                console.warn(
                  `   ⚠️  Failed to fetch cities for ${continent.slug}/${country.slug}: HTTP ${citiesResponse.status}`
                );
              }
            } catch (citiesError) {
              console.warn(
                `   ⚠️  Error fetching cities for ${continent.slug}/${country.slug}: ${citiesError.message}`
              );
            }
          }
        }
  ...
  printSummary() {
    ...
    console.log(
      `Routes: ${this.routes.length} × ${LOCALES.length} = ${this.routes.length * LOCALES.length}`
    );
