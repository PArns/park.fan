...
section('rectangles');
{
  ...
  /**
   * An anchor is not a centre.
   *
   * `tools/main.ts` turns `footprintOffset` into the rectangle's world centre, and this pins the
   * turn it uses. The bug it exists for: the ghost drew and judged a box centred on the pointer
   * while `track` built the machine FROM the pointer outwards, so the first coaster a player ever
   * placed came down 28 m east of the approved rectangle with 600 of 2001 track samples inside
   * the hill. The same convention as `rectCorners` above -- local +Z to world (sin, cos) -- and
   * it has to hold at every yaw, not only at zero, because rotating an offset item SWINGS its box.
   */
  const centre = (off, x, z, yaw) => {
    const [dx, dz] = off;
    const cos = Math.cos(yaw);
    const sin = Math.sin(yaw);
    return [x + dx * cos + dz * sin, z - dx * sin + dz * cos];
  };
  const straight = centre([28, 23.2], 100, -50, 0);
  near(straight[0], 128, 1e-9, 'an unrotated offset is added straight on');
  near(straight[1], -26.8, 1e-9, 'in z too');
  const quarter = centre([28, 23.2], 100, -50, Math.PI / 2);
  near(quarter[0], 123.2, 1e-9, 'a quarter turn sends the item’s +Z to world +X');
  near(quarter[1], -78, 1e-9, 'and its +X to world -Z');
  const halfTurn = centre([28, 23.2], 100, -50, Math.PI);
  near(halfTurn[0], 72, 1e-9, 'half a turn mirrors it through the anchor');
  near(halfTurn[1], -73.2, 1e-9, 'in both axes');
  const dist = Math.hypot(straight[0] - 100, straight[1] + 50);
  near(Math.hypot(quarter[0] - 100, quarter[1] + 50), dist, 1e-9, 'a turn moves the box, never resizes the reach');
  near(Math.hypot(halfTurn[0] - 100, halfTurn[1] + 50), dist, 1e-9, 'the same at half a turn');
}
...
section('palette');
{
  ...
  // Derived from `PALETTE_CATEGORIES`, not spelled out: this was a hand-written sum of five
  // manifest keys and it went red the day a sixth (`coasterLayouts`) was added — a test failing
  // on its own bookkeeping rather than on behaviour, which is the second one of those this week.
  const expected = bundled.reduce(
    (sum, p) => sum + PALETTE_CATEGORIES.reduce((n, category) => n + (p[category]?.length ?? 0), 0),
    0
  );
  ok(
