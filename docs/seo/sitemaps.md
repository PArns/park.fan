...
3. `/sitemap-calendar.xml` (`app/sitemap-calendar.xml/route.ts`) — the same shape again, over `/sitemap-calendar/<locale>.xml`, each holding that locale's **calendar month** URLs — today 3,816 per locale (212 parks × 18 months; the count moves as the archive fills, see below).
...
**The calendar months were split for the same reason, one build later.** They shipped inside `app/sitemap.ts` first, and that is how the ceiling got measured: 212 parks × 22 months × 6 locales was 27,984 entries, and with that file's seven-alternate `xhtml:link` block per URL the main sitemap came out at **39.5 MB against the 50 MB limit** — 32,748 URLs, so plenty of headroom on the count and almost none on the bytes. A file over the limit is rejected whole, which would have taken the park pages, the geo hubs, the glossary and the blog down with the calendar. Split out, `/sitemap.xml` is back to 4,764 URLs / 4.6 MB and each calendar child is 0.67 MB.
