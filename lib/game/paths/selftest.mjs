...
/**
 * The cost of a path query, measured in units of this machine at this moment.
 *
 * Two changes from the wall-clock version this replaces, and both were forced by the same failed
 * run: the suite went red here at `2.340 µs` against a 2 µs threshold while a critic agent was
 * driving four headless browsers, on code that measures 0.352-0.566 µs when nothing else is
 * running. A 6.4x spread on identical code is not a regression guard, it is a coin toss, and the
 * fix is not a looser number — the comment on the old constant is right that 2 µs is already
 * tuned to still catch the 20x regression this exists for (a `Map` walk back in the hot path),
 * so a threshold loose enough for a busy machine stops catching anything.
 *
 * So: **CPU time, not wall time** — `process.cpuUsage()` counts only cycles this process actually
 * spent, and excludes the time it sat descheduled waiting for a core, which is precisely what a
 * loaded machine adds. And then **a ratio, not an absolute** — the same lump of representative
 * work is timed the same way in the same run, so whatever is left (clock speed, cache pressure,
 * a different container) divides out. What the assertion below compares is therefore how many
 * calibration operations one path query costs, which is a property of the algorithm.
 */
const CAL_OPS = 400000;
function calibrate() {
  // Deliberately the shape of the hot path rather than an empty loop: a small `Map` read and a
  // little float arithmetic, which is what `next()` is made of. An empty loop would be optimised
  // to nothing and would scale differently under cache pressure than the thing it stands in for.
  const table = new Map();
  for (let i = 0; i < 64; i++) table.set(i, { x: i * 1.7 - 40, z: i * -0.3 + 6 });
  let acc = 0;
  const started = process.cpuUsage();
  for (let i = 0; i < CAL_OPS; i++) {
    const cell = table.get(i & 63);
    acc += Math.sqrt(cell.x * cell.x + cell.z * cell.z);
  }
  const spent = process.cpuUsage(started);
  // `acc` is returned so V8 cannot delete the loop that computed it.
  return { us: spent.user + spent.system, acc };
}

const QUERIES = 20000;
/**
 * THE FIRST CALL IS THROWN AWAY, and that line is the whole reason this measurement works.
 *
 * The first version of this calibration averaged one call taken before the queries with one taken
 * after, and read 31.7-61.7 ops per query over eight runs — worse than the wall clock it replaced,
 * and red on the fifth. Probed separately, twelve consecutive calls to the identical function cost
 * `10.58 2.83 2.87 2.93 3.36 2.95 2.84 2.79 2.90 2.92 2.86 2.77` ms of CPU: the first is JIT-cold
 * and 3.6x the rest, and warm calls spread only 1.03-1.21x across three probes. So the old average
 * mixed two regimes and called the result a machine speed.
 *
 * Warm it, then take the MINIMUM of the two warm readings — a scheduling hiccup can only make a
 * measurement of fixed work too slow, never too fast, so the minimum is the honest estimate of what
 * this machine does when it has a core.
 */
calibrate();
const calBefore = calibrate();
...
const cpuSpent = process.cpuUsage(cpu0);
const ms = performance.now() - t0;
const calAfter = calibrate();
const queryUs = cpuSpent.user + cpuSpent.system;
const calUs = Math.min(calBefore.us, calAfter.us);
// Calibration operations per query. `CAL_OPS / calUs` is operations per microsecond of CPU.
const opsPerQuery = (queryUs / QUERIES) * (CAL_OPS / calUs);
console.log(
  `  · ${QUERIES} next() in ${ms.toFixed(2)} ms wall, ${(queryUs / 1000).toFixed(2)} ms cpu ` +
    `(${(queryUs / QUERIES).toFixed(3)} µs each), ${answered} answered`
);
console.log(
  `  · calibration ${CAL_OPS} ops in ${(calUs / 1000).toFixed(2)} ms cpu ` +
    `→ ${opsPerQuery.toFixed(1)} calibration ops per query`
);
// The threshold comes from measurement, and the number means nothing on its own — only against the
// calibration loop above, so re-derive it if that loop ever changes. Eight runs of this file with
// two builder agents working alongside it: 86.8 · 89.5 · 90.2 · 90.6 · 102.8 · 106.1 · 119.4 ·
// 130.6 ops per query, over a calibration that held 2.78-3.41 ms across the same eight (1.23x).
// Four confirmation runs after the threshold landed added 83.6 · 103.2 · 123.5 · 139.7, so the
// honest observed range over all twelve is 83.6-139.7, a 1.67x spread. 400 leaves 2.9x over the
// worst of those, and a 20x regression — a `Map` walk back in the hot path, which is what this
// guard exists for — lands near 2,000.
check(
  'a path query stays inside its share of the tick',
  opsPerQuery < 400,
  `${opsPerQuery.toFixed(1)} calibration ops each, ${(queryUs / QUERIES).toFixed(3)} µs cpu, ` +
    `${ms.toFixed(2)} ms wall for ${QUERIES}`
);
