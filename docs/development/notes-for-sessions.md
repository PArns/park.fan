...
17. **The guide page** — `app/[locale]/how-park-fan-works/` (published on a localized slug per locale, `lib/howto/segments.ts`; the old `/howto` 301s). Content lives per locale in `content/[locale].tsx` and the top-level `page.tsx` only handles metadata and dispatches. German is the rewritten editorial version listed in `EDITORIAL_LOCALES`; the other five still render the previous feature manual in the legacy container, so check which branch you are editing. Its example UI is `_demos.tsx` (production components + `_fixtures.ts`, never a redrawn lookalike, never a fetch), its two motion pieces are `_wait-scale.tsx` and `_night-shift.tsx`, and `_howto-ui.tsx` / `_mock-components.tsx` are the legacy locales' primitives.
...
19. **Shared UI-Primitives (reuse-Regel, siehe CLAUDE.md)** — vor dem Inline-Bauen prüfen:
    ...
    - **Ganzes Kapitel einer Seite** → `PageSection` (`components/common/page-section.tsx`): rendert `<section>` + Kapitel-Überschrift + **das Spacing** (Top-Rhythmus `mt-10`, Abstand zum Content). Immer diese Komponente nehmen, nie `<section className="mt-10">` + `SectionHeading` von Hand — genau so ist das Spacing auseinandergelaufen. `id` setzt Anker + `scroll-mt-24`.
    - **Section-Header** (innerhalb eines Kapitels, oder wenn kein `<section>` gebraucht wird) → `SectionHeading` (`components/common/section-heading.tsx`): `variant="chapter"` (default, Icon-Kachel) für Kapitel einer Seite, `variant="plain"` + `as="h3"` (schlichtes Icon+Titel) für Karten-/Unterabschnitts-Titel _innerhalb_ eines Kapitels. `SectionHeader` gibt es nicht mehr. Weitere Props: `frosted` (Milchglas-Pill für Überschriften, die direkt auf dem Hero-Foto liegen — Park-/Ride-Seiten), `iconClassName` (abweichender Icon-Ton, z.B. Rope-Drop-Emerald), `hint` (gedämpfter Text rechts) und `badge` (fertiger Node, **kein** roher String — die Komponente wrappt nicht mehr selbst in `<Badge>`). Jede Section bekommt ein Icon; titellose Blöcke gibt es nicht.
