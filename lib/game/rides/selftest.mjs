...
}
{
  ...
  // One park hour, DERIVED from the clock constant and never typed.
  //
  // This read `rt.step(1200)` under a comment saying "20 ticks a second × 60 park minutes". D-006
  // made a park minute three real seconds instead of one, so those 1,200 ticks became twenty park
  // minutes, the carousel ran 6 cycles instead of 15 and carried 144 riders instead of its rated
  // load — two red assertions about a ride that was behaving perfectly. A tick count written by
  // hand measures the constant rather than the machine.
  const ONE_PARK_HOUR_TICKS = Math.round(60 / MINUTES_PER_TICK_AT_SPEED_1);
  rt.step(ONE_PARK_HOUR_TICKS);
...
// ── 9. machines this module does not draw ───────────────────────────────────────────────────
/**
 * A coaster and a flume, in a line this module runs.
 *
 * The gap this section exists for: `add()` opened with `if (entity.kind !== 'ride') return`, so
 * the queue, the height check, the balk and the boarding receipt — which exist exactly once, in
 * this file — never saw the game's headline object. Everything below drives the same five verbs
 * `guests` calls, against machines `trains` and `flumes` dispatch, and asserts the three things
 * that could not be true before: that a line forms at the STATION rather than at the entity, that
 * a receipt comes back with the ride's own duration in it, and that the load travels back to the
 * module that owns the vehicle.
 */
section('9 · a coaster and a flume');
{
  const world = createWorld({
    seed: 5,
    name: 'rides-docked',
  ...
  world.clock.minute = 12 * 60;
  attachFlumeContent(freshRegistry());
  const preset = TRACK_LAYOUTS.find((p) => p.id === 'kleiner-kreisel');
  const origin = [0, 8, 0];
  const yaw = -Math.PI / 2;
  world.entities.coaster = {
    id: 'coaster',
    kind: 'coaster',
    pack: 'core-classic',
    item: 'family-invert',
  ...
  world.entities.slide = makeFlumeEntity({
    id: 'slide',
    pack: 'neon-lagoon',
    item: 'tube-slide',
    x: 200,
  ...
  rt.init({
    ...
    world,
    packs: PACKS,
    modules: ['core', 'terrain', 'track', 'trains', 'pools', 'flumes', 'rides'],
  });
  rt.step(4);
  const api = rt.handles.get('rides').api;
  const trains = rt.handles.get('trains').api;

  const views = api.list();
  ok(views.length === 3, 'all three machines have a line', String(views.length));
  const byId = Object.fromEntries(views.map((v) => [v.id, v]));
  ok(byId.coaster?.dispatchedBy === 'trains', 'the coaster is dispatched by trains');
  ok(byId.slide?.dispatchedBy === 'flumes', 'the flume is dispatched by flumes');
  ok(byId.flat?.dispatchedBy === 'rides', 'the flat ride dispatches itself');
  ok(
    api.roster().length === 1 && api.roster()[0] === 'flat',
    'and only the flat ride is in the frame roster',
    JSON.stringify(api.roster())
  );

  // The line is at the STATION, which is not where the entity is. A venue built at the entity
  // would walk every guest to the wrong end of a 610 m layout.
  const offer = api.offer('coaster');
  const away = Math.hypot(offer.queueX - origin[0], offer.queueZ - origin[2]);
  ok(away > 5, 'the coaster queue is off the layout origin', `${away.toFixed(1)} m`);
  ok(away < 40, 'and still beside its own station', `${away.toFixed(1)} m`);
  ok(offer.open, 'the coaster is open at noon');
  ok(
    offer.throughput * 60 > 400,
    'its nameplate comes from the fleet, not from a manifest',
    `${Math.round(offer.throughput * 60)}/h`
  );
  const slideOffer = api.offer('slide');
  const slideAway = Math.hypot(slideOffer.queueX - 200, slideOffer.queueZ - 0);
  ok(
    slideAway > 2 && slideAway < 20,
    'the flume queue is at the foot of its tower',
    `${slideAway.toFixed(1)} m`
  );

  // A child too short for a 100 cm limit is turned away, by the same code path a carousel uses.
  ok(api.join('coaster', 900, { heightCm: 90 }) == null, 'a short child is refused');
  ok(api.lastRefusal('coaster') === 'too-short', 'and the reason names the height limit');

  const join = api.join('coaster', 901, { heightCm: 150 });
  ok(join != null && join.ticket > 0, 'a guest can join a coaster queue');
  ok(api.place('coaster', join.ticket) != null, 'and is told where to stand');
  let receipt = null;
  for (let i = 0; i < 400 && !receipt; i++) {
    rt.step(1);
    receipt = api.board('coaster', join.ticket);
  }
  ok(receipt != null, 'and boards a train');
  ok(
    receipt != null && receipt.rideMinutes > 0.2,
    'with the lap time in the receipt',
    `${receipt?.rideMinutes?.toFixed(2)} min`
  );
  ok(
    receipt != null && receipt.satisfaction > 0,
    'and an opinion about it',
    String(receipt?.satisfaction)
  );
  api.leave('coaster', join.ticket);
  ok(api.place('coaster', join.ticket) == null, 'and can be taken off again');

  // A park hour of demand, and the two machines carry it.
  rt.command({ type: 'rides:demo', seq: 1, payload: { on: true } });
  const ONE_HOUR = Math.round(60 / MINUTES_PER_TICK_AT_SPEED_1);
  rt.step(ONE_HOUR);
  const after = Object.fromEntries(api.list().map((v) => [v.id, v]));
  ok(
    after.coaster.ridersToday > 100,
    'the coaster carries a park hour of riders',
    String(after.coaster.ridersToday)
  );
  ok(after.slide.ridersToday > 20, 'and so does the flume', String(after.slide.ridersToday));
  ok(
    after.coaster.ridersToday <= after.coaster.ratedThroughput * 1.02,
    'neither beats its own nameplate',
    `${after.coaster.ridersToday} vs ${Math.round(after.coaster.ratedThroughput)}`
  );
  ok(
    trains.status('coaster').riders > 0,
    'and the load travelled back to the fleet',
    String(trains.status('coaster').riders)
  );
  ok(
    trains.status('coaster').riders <= trains.status('coaster').seats,
    'never more than the train holds'
  );
  console.log(
    `    coaster ${after.coaster.ridersToday} riders/h (rated ${Math.round(after.coaster.ratedThroughput)}) · ` +
      `flume ${after.slide.ridersToday} (rated ${Math.round(after.slide.ratedThroughput)}) · ` +
      `carousel ${after.flat.ridersToday}`
  );
  ok(messages.filter((m) => m.type === 'error').length === 0, 'no sim errors');

  // The save still round-trips with two machines this module does not draw in it.
  const json = rt.serialize();
  ok(serializeWorld(SimRuntime.parse(json)) === json, 'a world with a coaster queue round-trips');
  const slot = rt.handles.get('rides').serialize();
  const saved = slot.rides.find((r) => r.id === 'coaster');
  ok(saved != null, 'the coaster has a slot of its own in the save');
  ok(Array.isArray(saved?.onboard), 'with the people on it in it');
  rt.dispose();
}

// ── 10. the same world twice ────────────────────────────────────────────────────────────────
section('10 · docked determinism');
{
  const build = () => {
    const world = createWorld({
      seed: 9,
      name: 'rides-docked-det',
      packs: ['core-classic', 'neon-lagoon'],
    });
    world.clock.minute = 12 * 60;
    const preset = TRACK_LAYOUTS.find((p) => p.id === 'kleiner-kreisel');
    ...
    rt.init({
      ...
      world,
      packs: PACKS,
      modules: ['core', 'terrain', 'track', 'trains', 'rides'],
    });
    rt.command({ type: 'rides:demo', seq: 1, payload: { on: true } });
    rt.step(900);
    return rt;
  ...
  const a = build();
  const b = build();
  const sa = JSON.stringify(a.handles.get('rides').serialize());
  const sb = JSON.stringify(b.handles.get('rides').serialize());
  ok(sa === sb, 'two runs of one seed write the same bytes with a coaster in them');
  const ta = JSON.stringify(a.handles.get('trains').serialize());
  const tb = JSON.stringify(b.handles.get('trains').serialize());
  ok(ta === tb, 'and so does the fleet that carried them');
  // A resumed run and an uninterrupted one agree, which is the gate a boarding count could break.
  ...
  b.step(300);
  ok(
    JSON.stringify(resumed.handles.get('rides').serialize()) ===
      JSON.stringify(b.handles.get('rides').serialize()),
    'a resumed run and an uninterrupted one stay identical'
  );
  a.dispose();
  b.dispose();
  resumed.dispose();
