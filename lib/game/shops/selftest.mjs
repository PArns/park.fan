...

{
  ...
  //
  // The count is DERIVED from the clock constant, never typed. This used to read `3000` with a
  // comment saying "3,000 ticks at speed 5 is 750 park minutes"; D-006 changed a park minute from
  // one real second to three, the same 3,000 ticks became 250 park minutes, the clock stopped at
  // about 13:10 and the assertion below failed on shops that were correctly still open. A test that
  // hard-codes a tick count is a test that measures the constant instead of the behaviour.
  const TRADING_MINUTES = 750; // 11:00 → 23:30, past the burger stand's 22:30 close
  const ticks = Math.round(TRADING_MINUTES / (world.clock.speed * MINUTES_PER_TICK_AT_SPEED_1));
  for (let i = 0; i < ticks; i++) rt.scheduler.step();
  const stats = api.stats();
  console.log(
    `  · after ${ticks.toLocaleString('en-GB')} ticks (${TRADING_MINUTES} park minutes): served ${stats.servedToday}, takings ${(stats.takingsToday / 100).toFixed(2)}, queue ${stats.queue}, refused ${JSON.stringify(stats.refusedToday)}`
  );
  ...
  // The clock is at 23:30 now and the burger stand's own hours (09:30–22:30) have closed it. Every
  // need is unanswered, which is the right answer and is why the assertion above is taken at 11:00
  // rather than here.
  check(
