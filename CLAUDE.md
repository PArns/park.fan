...
| Working on …                                                                          | Open                                                       |
| ------------------------------------------------------------------------------------- | ---------------------------------------------------------- |
| page payload, caching, `revalidate`, streaming, CLS, translations reaching the client | [architecture rules](docs/rules/architecture.md)           |
| wait times, seasons, a park with no source                                            | [data and API rules](docs/rules/data-and-api.md)           |
| trip planner, header menu, admin, weather chart, guide page, glossary, blog manifest  | [feature rules](docs/rules/features.md)                    |
| layout, a component, a breakpoint, the header, a card, three.js                       | [design rules](docs/rules/design.md)                       |
| sitemaps, feeds, favicon, robots and agents, analytics                                | [SEO rules](docs/rules/seo.md)                             |
| a blog post, UI strings, images, captions                                             | [content and media rules](docs/rules/content-and-media.md) |

## Applies to every change

- **Reuse existing components (REQUIREMENT):** always reuse what exists (`ParkStatusBadge`,
...
- **No text may read as AI-generated (REQUIREMENT):** this governs every string a human ever sees —
  ...
  [content and media rules](docs/rules/content-and-media.md#no-text-may-read-as-ai-generated-requirement)
...
### Architecture and payload → [docs/rules/architecture.md](docs/rules/architecture.md)

- **Park page loading priority** — best-travel-time data (best-days calendar + historical stats) loads
  **last**; live status, wait times and every weather query go first. Enforced by `useLoadLast`
  (`lib/hooks/use-load-last.ts`).
- **A streamed section owes the page its height** — a `<Suspense>` fallback reserves the height its
...
- **API budget per page** — the 5-minute live poll sends `LiveParkSnapshot`, a projection, never the
...
- **The page render is the bigger half of the API budget (ANTI-PATTERN)** — the RSC payload is roughly
  ...
  cache-busted URL.
- **A `revalidate` at a call site is somebody else's page** — Next takes the shortest `revalidate` a
...
- **Translations are routed, not bundled** — the locale layout ships only the chrome; each route adds
...
### Data and API → [docs/rules/data-and-api.md](docs/rules/data-and-api.md)

- **Parks we cannot read** — a park with no wait-time source is byte-for-byte a park shut for the
...
- **A ride out of season is closed, and is not one of the park's rides today** — `isCurrentlyInSeason`
...
### Features → [docs/rules/features.md](docs/rules/features.md)

- **The planner's day ends when the park closes, and a headliner is not a residual** —
...
- **A day that does not fit opens an assistant, not a footnote** — both presses probe with
...
- **Weather day chart is built around the park's hours** — the time axis is piecewise linear and both
...
- **The admin holds no credential** — an httpOnly session cookie; `adminFetch` is a plain same-origin
  ...
  `success: true`.
- **The header menu is three kinds of content, and the split is about the link graph** — the parks
...
- **The guide page teaches the real cards with the ride's real numbers** — every block renders a
...
- **Ride ↔ Glossary link** — `rideProfile` figures are glossary term ids **in ride order**; repeats
  are intentional. Never dedupe or sort. This app is the only place a term id is defined.
- **Blog manifest is split** — import listings from `@/lib/blog/listing`; `@/lib/blog` drags every
  post body into the bundle of a route the root layout imports.

### Design and layout → [docs/rules/design.md](docs/rules/design.md)

- **A chapter opens the same way everywhere** — one component draws every chapter header,
...
- **A client-only preference may not decide server-rendered markup** — temperatures render in both
  units and CSS shows one. Hydration is not one pass, so the guard belongs to the consumer, not the
  provider.
- **A cell is gated on its content, and a component that fills one may not return `null` — nor one
  line** — make the component total (`ropeDropCardVariant()`), or export the predicate so the count
...
- **The header is 48 px, and its height is written down in four places** — control heights come off
...
- **three.js animations** — research the real-world reference **first**, then verify every animation
  from all camera perspectives and both themes via `node scripts/render-coaster-elements.mjs`.
  A green build is not enough.
- **The quietest weekday may be two days, and a thin day drops out rather than ending the vote** —
...
- **Blog spotlight cards** — the row template sits on the card itself, never on a shared wrapper that
  also holds the heading.
- **A blog card is a row on phones** — below `sm` the card is not rendered at all; `BlogPostRow` is.
...
### SEO and the machine-facing surface → [docs/rules/seo.md](docs/rules/seo.md)

- **The favicon is generated from two sources, and it carries no wordmark** — the simple pin for
...
- **Nothing here has a reader inside the site** — robots, `llms.txt`, the `.well-known` documents,
...
- **A `<lastmod>` is observed, never stamped** — the fingerprint covers what a reader sees and what
...
- **A feed nobody links to is a file with a URL** — every route carries its own
...
- **Umami event budget** — every event property is billed as another event. Never send what Umami
  already has, never what a sibling property implies, and price load-fired events by their property
  count.

### Content and media → [docs/rules/content-and-media.md](docs/rules/content-and-media.md)

- **No text may read as AI-generated** — see _Applies to every change_ above. The full rulebook is
  [docs/blog.md](docs/blog.md).
- **Blog writing style** — never „ehrlich" in any form, no em dash in German running text, and check
  the article before correcting one in (**das** Efteling).
- **A wait time is never typed into a post** — every such table is a fence (`ride-waits-widget`,
  `hourly-profile-widget`). A sentence next to a widget must not name a figure the widget renders.
- **Park/Ride page ↔ blog link** — the relation is derived from the post itself; `parkLinks` and
  `rideLinks` override it, independently of each other and **per post, not per locale**.
- **Media database** — one filesystem-backed database under `public/media/<collection>/`, and the
...
- **One photo can index two rides (`alsoRides`)** — names the further rides a photo answers for, every
  slug validated against the park's attractions.
- **Card photos are two layers** — the framing reference is the strip between the glass panels
  (`CardPhotoFrame`), never the whole card. `pnpm check:card-framing`.
- **Localized blog gallery captions** — a gallery is a collection, and its captions live per image in
  the sidecar.
...
| Features        | [admin](docs/features/admin.md) · [media database](docs/features/media-database.md) · [glossary](docs/features/glossary.md) · [the guide page](docs/features/how-park-fan-works.md) · [trip planner](docs/features/trip-planner.md)                                                                                                                                                                                                                   |
